...
export function AvatarInput({ index, isSelected = false, onSelect, defaultChecked }: AvatarInputProps) {
  return (
    <label className={`relative justify-self-center rounded-full border-4  ${isSelected ? ' border-yellow-500' : 'border-transparent'}`} onClick={onSelect}>
      <input type="radio" defaultChecked={defaultChecked} name="avatar" className="hidden" value={`avatar_${index}.png`} />
